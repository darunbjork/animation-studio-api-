...
async function runWorker() {
  ...
  pipelineQueue.process(2, async (job) => {
    ...
    logger.info(`Processing pipeline for asset`, {
      correlationId,
      jobId: job.id,
      assetId,
      version,
    ...
    if (!pipeline) {
      logger.error(`Pipeline for asset not found.`, {
        correlationId,
        assetId,
        version,
        jobId: job.id,
    ...
    try {
      ...
      await pipeline.save();
      emitUpdate();
      logger.info(`Pipeline PROCESSING_PREVIEW`, {
        correlationId,
        pipelineId: pipeline.id,
        assetId,
        version,
      ...
      await pipeline.save();
      emitUpdate();
      logger.info(`Pipeline READY_FOR_RENDER`, {
        correlationId,
        pipelineId: pipeline.id,
        assetId,
        version,
      ...
      await pipeline.save();
      emitUpdate();
      logger.info(`Pipeline RENDER_QUEUED`, {
        correlationId,
        pipelineId: pipeline.id,
        assetId,
        version,
...
runWorker().catch((err) => {
  logger.error('Pipeline worker failed to start:', {
    error: err.message,
    stack: err.stack,
  });
  process.exit(1);
