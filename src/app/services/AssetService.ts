...
export class AssetService {
  static async createAsset(data: any, correlationId?: string) {
    if (!data.name) {
      logger.warn("Asset name is required for asset creation", { correlationId, data });
      throw new ValidationError("Asset name is required");
    }

    const asset = await AssetRepository.create(data);
    logger.info("Asset created in service", { correlationId, assetId: asset._id, studioId: asset.studioId });
  ...
  static async getAsset(assetId: string, studioId: string, correlationId?: string) {
    ...
    if (!asset) {
      logger.warn("Asset not found for retrieval", { correlationId, assetId, studioId });
      throw new ValidationError("Asset not found");
    }
    logger.info("Asset retrieved in service", { correlationId, assetId, studioId });
    return asset;
  }

  static async updateAsset(assetId: string, studioId: string, data: any, correlationId?: string) {
    const asset = await AssetRepository.update(assetId, studioId, data);

    if (!asset) {
      logger.warn("Asset not found for update", { correlationId, assetId, studioId, data });
      throw new ValidationError("Asset not found");
    }
    logger.info("Asset updated in service", { correlationId, assetId, studioId, changes: data });
  ...
  static async deleteAsset(assetId: string, studioId: string, userRole: string, correlationId?: string) {
    if (!PermissionService.canDelete(userRole as any)) { // Cast to Role for now, will fix with proper type later
      logger.warn("User unauthorized to delete asset", { correlationId, assetId, studioId, userRole });
    ...
    if (!asset) {
      logger.warn("Asset not found for deletion", { correlationId, assetId, studioId });
      throw new ValidationError("Asset not found");
    }
    logger.info("Asset deleted in service", { correlationId, assetId, studioId });
  ...
  static async rollbackAsset(
    ...
    if (!asset) {
      logger.warn("Asset not found for rollback", { correlationId, assetId, studioId, version });
      throw new ValidationError("Asset not found");
    }

    asset.currentVersion = version;
    await asset.save();
    logger.info("Asset rollback executed in service", { correlationId, assetId, studioId, newVersion: version });
  }
}
